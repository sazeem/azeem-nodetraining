...
exports.getCommitList = (req,res) => {  
  commits.findAll()
   .then((commits) => requestGitHub(commits,req,res));
}

const requestGitHub = (commits,req,res) => {
...
const storeCommits = (repos,myResponse,repoName,res) => {                    
  let myCommits = mapper(myResponse,repos,repoName,res);
...
const mapper = (myResponse,repos,repoName,res) => {
    let newRepository = [];
    try{              
